...
- **v2 — enrichissement.** *Aide à la lecture des graphiques **livrée** (bloc `CTX_MODULE_VIZ`, 22/07). **Scope écran live (feature B) livrée** le 23/07 — engine `6241` en prod, cf. [[Session 2026-07-23]].* Reste : tools saturation/allocation (`cf-budget-allocator-prod`), enrichissement `BUSINESS_CONTEXT` par les DS, contexte/knowledge par client. Backlog priorisé dans [[Session 2026-07-22]].
