...
| Sujet                                                           | État                                         | Détail                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| --------------------------------------------------------------- | -------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `no_answer` (mode 1) — hallucination de tool `schema_agent`     | 🟠 root cause vérifiée, durcissement à faire | **Root cause (vérifiée 08/07)** : `schema_agent` (Flash) hallucine un appel `execute_sql` — tool réel du pipeline (`executor_agent`, `agent.py:370`) mais **hors de son toolset** (`schema_agent` n'a que `get_table_info`, `agent.py:244`). ADK renvoie **498** → stream mort → `no_answer` (catch-all `ChatbotError` front). **Non-déterministe** (hallucination LLM) → 1 alerte, pas de panne totale ; re-run passe souvent. **Suspect** : le `BUSINESS_CONTEXT` injecté dans l'instruction de `schema_agent` (`agent.py:242`) décrit le pipeline SQL complet → amorce l'agent « schéma-only » à tenter `execute_sql`. **Fix candidats** : (1) scoper le `BUSINESS_CONTEXT` par agent (retirer l'exécution pour `schema_agent` — compatible « enrichir pour la qualité » : scoping, pas allègement global) ; (2) instruction « `get_table_info` only » ; (3) retry backend sur 498 / stream vide. Supersede l'hypothèse « routage définitionnel » (même symptôme, mauvais diagnostic). |
| `no_answer` (mode 2) — raté transitoire gatekeeper              | 🟠 observé en prod, filet backend à faire    | **Observé 10/07** (session `1008426160385687552`, client Peugeot `6a61e3`, question « Compare the ROAS of TV, display and search over the last period ») : à 14:29:25 `MMM_agent` émet un event **sans parts** — ni texte, ni `transfer_to_agent` → stream mort → `no_answer`. C'est le **point de mort le plus amont** (avant `context_setter_agent`), **distinct du mode 1** (qui meurt sur `schema_agent`, plus loin). Signature d'alerte : `authors = MMM_agent` seul. **Non-déterministe** : re-run de la **même** question 4 min après (14:33) → gatekeeper transfère normalement, pipeline OK, réponse produite (14:34:51). **Aucun fix logique côté agent** (échec amont transitoire, LLM/ADK) → seul filet possible : **retry backend** sur réponse gatekeeper vide / stream terminé sans `done`. Diagnostic via **Sessions API rétroactive** (recette §4) — validée sur ce cas.                                                                                                 |
...
| README repo périmé                                              | 🟢 mineur                                    | ID resource `4105…` au lieu du live `6073…`. Repo perso, non prioritaire.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
