...
> **Les bugs majeurs sont résolus.** Le `no_answer` par hallucination de tool (`execute_sql`) — le problème central — est **corrigé** (scoping `CTX_*` + filet retry, 20-22/07). Restent **2 limites ouvertes** : le faux négatif de `validate_client_filter` et les sessions backend **en mémoire**. Le reste du tableau est historique (✅).

| Sujet                                                           | État                                         | Détail                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| --------------------------------------------------------------- | -------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `no_answer` (mode 1) — hallucination de tool `schema_agent`     | ✅ corrigé (20-22/07) | **Root cause (vérifiée 08/07)** : `schema_agent` (Flash) hallucine un appel `execute_sql` — tool réel du pipeline (`executor_agent`, `agent.py:370`) mais **hors de son toolset** (`schema_agent` n'a que `get_table_info`, `agent.py:244`). ADK renvoie **498** → stream mort → `no_answer` (catch-all `ChatbotError` front). **Non-déterministe** (hallucination LLM) → 1 alerte, pas de panne totale ; re-run passe souvent. **Suspect** : le `BUSINESS_CONTEXT` injecté dans l'instruction de `schema_agent` (`agent.py:242`) décrit le pipeline SQL complet → amorce l'agent « schéma-only » à tenter `execute_sql`. **Fix livré (20-22/07)** : scoping `BUSINESS_CONTEXT` par sous-agent (schema → aucun contexte) + « one tool only » + **filet retry backend** (§3). Le bug **migre** (schema → discovery → `context_setter_agent`, ce dernier à **zéro** priming) → hallucination Flash **non-déterministe** → le retry est le fix **uniforme**, au-delà du scoping. |
| `no_answer` (mode 2) — raté transitoire gatekeeper              | ✅ corrigé (retry, 22/07)    | **Observé 10/07** (session `1008426160385687552`, client Peugeot `6a61e3`, question « Compare the ROAS of TV, display and search over the last period ») : à 14:29:25 `MMM_agent` émet un event **sans parts** — ni texte, ni `transfer_to_agent` → stream mort → `no_answer`. C'est le **point de mort le plus amont** (avant `context_setter_agent`), **distinct du mode 1** (qui meurt sur `schema_agent`, plus loin). Signature d'alerte : `authors = MMM_agent` seul. **Non-déterministe** : re-run de la **même** question 4 min après (14:33) → gatekeeper transfère normalement, pipeline OK, réponse produite (14:34:51). **Corrigé par le filet retry backend** (§3) : ce mode 2 (mort sur `MMM_agent` seul) et le mode 1 sont le **même** phénomène non-déterministe → un retry uniforme les couvre. Diagnostic via **Sessions API rétroactive** (recette §4) — validée sur ce cas.                                                                                                 |
| `validate_client_filter` — faux négatif (garde-fou d'isolation) | 🟠 à creuser                                 | **Observé 10/07** (même session, retry Q4) : `filter_checker_agent` renvoie **`SQL APPROVED`** puis `executor_agent` répond « Execution skipped — SQL did not pass the client filter check » alors que `WHERE t.client_id = '6a61e3'` **est présent** ; `validator_agent` confirme la contradiction. Brûle une itération du `refinement_loop` (la question finit par répondre à la 3ᵉ itération, via une reformulation en CTE). **Sur-blocage, pas de fuite** : le garde-fou refuse une requête légitime → problème de **fiabilité**, pas de sécurité. **Piste** : forme aliasée `t.client_id = '…'` (rejetée ici) vs `client_id = '…'` nu dans un CTE (acceptée au retry) — **mais** la même forme aliasée a été acceptée plus tôt dans la session (events Q1/Q2/Q3) → composante **non-déterministe / d'état** probable, pas purement syntaxique. **À faire** : logguer l'input exact vu par `validate_client_filter` (Python) au moment du refus pour trancher.                        |
| `reachedAnswerAgent` toujours vrai                              | ✅ corrigé                                    | Comptait `MMM_agent` (toujours présent). Fix : PR #1647 (develop) + #1648 (main), 08/07.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| README repo périmé                                              | ✅ corrigé                                    | ID corrigé (`4105…`→`6073…`) + **table de mapping engine↔commit** ajoutée (20/07), tenue à jour jusqu'à `2659…`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
