...
⚠️ **Correction du 23/07 (soir)** : en refermant la question « colonne marché » plus tôt dans la journée, j'avais conclu qu'il n'y avait pas de dimension marché et que `level` était une granularité d'analyse. **C'est inexact.** La lecture de la source de `cf-budget-allocator-prod` montre qu'elle fait `level = request_json["market"]` puis requête `tb_model_configuration WHERE … AND level = …` ; et le front groupe `data.results` par `d.level` pour construire ses `markets_scopes`. **`level` est bien la clé marché** dans tout le système, et la description de `CTX_FIELDS` (« granularité de l'analyse, global ou local ») est au mieux incomplète. Retirer `market` des règles SQL reste le bon choix pour l'agent (sa table est `tb_model_contributions`, et aucun client multi-marché n'existe aujourd'hui), mais **pas pour la raison invoquée**. À reprendre si un client multi-marché arrive, et à corriger dans `CTX_FIELDS`.
